...
module.exports = (program) => {
  ...
  get
    ...
    .description('returns information about ADAMANT network address (account), information returned consists of balance and publicKey.')
    .action((address) => log.call(api.getAddress)(address));

  get
    .command('block <blockId>')
    .description('returns block information, which contains information about forger (generatorId), timestamp, signatures, and other fields.')
    .action((id) => log.call(api.getBlock)(id));

  get
    .command('blocks <query>')
    .description('returns array of blocks in ADAMANT chain from newest to oldest.')
  ...
  get
    ...
    .description('returns information about message and the message itself decoded. Works the same way as get transaction, but returns asset decoded.')
